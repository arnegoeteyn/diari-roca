...
import { Route, RouteWithAscents } from "@/lib/routes/types";
...
export default function useRoutes(
  props?: Props
): [RouteWithAscents[], boolean] {
  const [routes, setRoutes] = useState<RouteWithAscents[]>([]);
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const transaction = await startTransaction(["ascents", "routes"]);

      let routes = [];

      routes = await Promise.all(
        routeKeys.map(async (key) => {
          const route = getRoute(key, transaction);
          return route;
        })
      );
